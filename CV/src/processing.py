...
def visualize_results(data, type, epochs):
    types={"loss": (0, 2), "metric": (1, 3)}
    current_type=types.get(type, -1)
    if(current_type != -1):
        plt.figure(figsize=(6, 4))
        plt.plot([val[current_type[0]] for val in data], label=f"train {type}", color="orange")
        plt.plot([val[current_type[1]] for val in data], label=f"valid {type}", color="blue")
        plt.grid(True)
        plt.xlim([0, epochs-1])
        plt.legend(loc="best")
        plt.title(f"{type.capitalize()} dynamics")
        plt.xlabel("№ epoch")
        plt.ylabel(f"{type.capitalize()} value")
        plt.show()
    else:
        raise ValueError("Such type isn't exists")